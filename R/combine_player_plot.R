...
combine_player_plot.default <- function(combine_profile, ...) {
  ...
  p <- combine_plot_base(combine_profile, player_combine_points) +
    annotate("text", x = 1, y = 1,
             ...
             colour = "white") +
    geom_point(data = player_combine_points,
               aes(x = time_splits, y = combine_accel, colour = player_label),
               show.legend = TRUE, size = 1.5,alpha = 0.5) +
    geom_point(data = player_combine_points,
               aes(x = time_splits, y = combine_speed, colour = player_label),
...
combine_plot_base <- function(player_profile, player_combine_points) {
  ...
  ggplot() +
    ...
    labs(title = "40 yd/ 36.6 m Speed and Acceleration Curves",
